...
import { AppSettings, VisibilityGroup, saveAppSettings } from '../lib/appSettings';
import { AreaCategory, CuisineCategory, sortByOrder, reorder } from '../lib/categories';
import { UsernameEntry } from '../lib/auth';
import { resyncPrivateRestaurantsWithVisibilityGroups } from '../lib/db';
...
export function AdminSettingsPage({
  ...
  const handleAddVisibilityGroup = () => {
    if (!isAdmin) return;
    const id = crypto.randomUUID();
    setDraft(prev => ({
      ...prev,
      visibilityGroups: [...prev.visibilityGroups, { id, name: '新しいグループ', uids: [] }],
    }));
    setSaveState('idle');
  };

  const handleRenameVisibilityGroup = (id: string, name: string) => {
    if (!isAdmin) return;
    setDraft(prev => ({
      ...prev,
      visibilityGroups: prev.visibilityGroups.map(g => (g.id === id ? { ...g, name } : g)),
    }));
    setSaveState('idle');
  };

  const handleToggleVisibilityGroupMember = (id: string, uid: string) => {
    if (!isAdmin) return;
    setDraft(prev => ({
      ...prev,
      visibilityGroups: prev.visibilityGroups.map(g =>
        g.id === id
          ? { ...g, uids: g.uids.includes(uid) ? g.uids.filter(u => u !== uid) : [...g.uids, uid] }
          : g,
      ),
    }));
    setSaveState('idle');
  ...
  const handleDeleteVisibilityGroup = (id: string, name: string) => {
    if (!isAdmin) return;
    if (
      !window.confirm(
    ...
    setDraft(prev => ({ ...prev, visibilityGroups: prev.visibilityGroups.filter(g => g.id !== id) }));
    setSaveState('idle');
  };
  ...
  const handleSaveAll = async () => {
    ...
    const trimmedVisibilityGroups = draft.visibilityGroups.map(g => ({ ...g, name: g.name.trim() }));
    if (
      trimmedCategories.some(c => !c.name) ||
      trimmedCuisineCategories.some(c => !c.name) ||
      trimmedVisibilityGroups.some(g => !g.name)
    ) {
      setSaveError('カテゴリ名・グループ名を空にすることはできません');
      return;
    }
    ...
    const nextDraft: AppSettings = {
      ...
      visibilityGroups: trimmedVisibilityGroups,
    };
    setSaveState('saving');
    setSaveError('');
    try {
      await saveAppSettings(nextDraft);
      await resyncPrivateRestaurantsWithVisibilityGroups(nextDraft.visibilityGroups);
      setDraft(nextDraft);
      onSaved(nextDraft);
      setSaveState('saved');
    } catch (error) {
